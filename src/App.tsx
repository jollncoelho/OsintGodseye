...
export default function App() {
  ...
  const { cameras: liveCameras, loading: cctvLoading, error: cctvError } = useCctvCameras(layers.cctv);

  const allCctv = useMemo(() => [...MOCK_CCTV, ...liveCameras], [liveCameras]);
  ...
  useEffect(() => {
    if (cctvError) addLog('warn', `TfL JamCam proxy error: ${cctvError} — using static cameras`);
    else if (!cctvLoading && liveCameras.length > 0) addLog('info', `TfL JamCam: ${liveCameras.length} live cameras loaded`);
  }, [liveCameras, cctvLoading, cctvError, addLog]);
